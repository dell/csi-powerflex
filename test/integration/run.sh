...
GOOS=linux CGO_ENABLED=0 GO111MODULE=on go test -v -coverprofile=c.linux.out -timeout 60m -coverpkg=github.com/Ecosystems/container-storage-modules/src/csi-vxflexos/service -run "^$testRun\$\$" &
