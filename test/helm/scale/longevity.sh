...
helm install --set "name=SP-SW_HDD-1,namespace=$ns,replicas=$replicas,storageClass=vxflexos"  -n SP-SW_HDD-1 --namespace $ns  $helmDir
...
helm upgrade --set "name=SP-SW_HDD-1,namespace=$ns,replicas=$1,storageClass=vxflexos"  SP-SW_HDD-1 --namespace $ns  $helmDir
