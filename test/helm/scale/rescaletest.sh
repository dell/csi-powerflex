...
helm upgrade --set "name=pool1,namespace=test,replicas=$replicas,storageClass=vxflexos"  pool1 --namespace test  10replicas
helm upgrade --set "name=pool2,namespace=test,replicas=$replicas,storageClass=vxflexos-pool2"  pool2 --namespace test  10replicas
helm upgrade --set "name=pool3,namespace=test,replicas=$replicas,storageClass=vxflexos-pool3"  pool3 --namespace test  10replicas
