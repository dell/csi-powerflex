...
var _ = ginkgo.Describe("Nodes Scale-up and Scale-down", ginkgo.Label("csi-fsg"), ginkgo.Label("csi-scale"), func() {
	...
	*/
	ginkgo.It("[csi-adv-fsg] Statefulset with Nodes Scale-up and Scale-down", func() {
		...
		ssPodsBeforeScaleDown, _ := fss.GetPodList(ctx, client, statefulset)
		...
		ssPodsAfterScaleDown, _ := fss.GetPodList(ctx, client, statefulset)
		...
		fss.WaitForStatusReadyReplicas(ctx, client, statefulset, replicas)
		ssPodsAfterScaleUp, _ := fss.GetPodList(ctx, client, statefulset)
