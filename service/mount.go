...
// Variable for FS check event recording
var mountFsCheckEventRecorder record.EventRecorder

var (
	fsCheckEventRecorderOnce   sync.Once
	cachedFsCheckEventRecorder record.EventRecorder
)

// newFsCheckEventRecorder creates a record.EventRecorder for FS check events.
var newFsCheckEventRecorder = func() (record.EventRecorder, error) {
	...
	return eventBroadcaster.NewRecorder(scheme, corev1.EventSource{Component: "csi-vxflexos-node"}), nil
}
...
func initFsCheckEventRecorder() record.EventRecorder {
	...
	return cachedFsCheckEventRecorder
}
...
func runPreMountFsck(
	...
	observer := &fsCheckPVCObserver{
		...
		fsType:        fsType,
		logFields:     f,
		eventRecorder: mountFsCheckEventRecorder,
		timedOut:      false,
	}
	...
	if err != nil {
		...
		errMsg := fmt.Sprintf("File system check failed on device %s (volume ID: %s, fs: %s): %v. Manual intervention required. Do not attempt to mount this volume until the file system has been repaired.", devicePath, volumeID, fsType, err)

		if observer.eventRecorder != nil && observer.pvcName != "" && observer.pvcNamespace != "" {
			pvcRef := &corev1.ObjectReference{
				Kind:      "PersistentVolumeClaim",
				Name:      observer.pvcName,
				Namespace: observer.pvcNamespace,
			}
...
func publishVolume(
	...
	targetMnts, err := getPathMounts(ctx, target)
	if err != nil {
		return status.Errorf(codes.Internal,
			"could not reliably determine existing mount status: %s",
			err.Error())
...
// publishNVMEVolume bind mounts the nvme device to the target path
func publishNVMEVolume(req *csi.NodePublishVolumeRequest, reqID, devicePath string) error {
	...
	ctx := context.WithValue(context.Background(), gofsutil.ContextKey("RequestID"), reqID)
	published, err := isAlreadyPublished(ctx, targetPath)
	if err != nil {
		return err
	}
	...
	if stagingPath == "" {
		return status.Error(codes.InvalidArgument, "staging target path required")
	}
	...
	isBlock, mntVol, accMode, _, err := validateVolumeCapability(volCap, ro)
	if err != nil {
		return err
	}
	...
	if isBlock {
		...
		if err != nil {
			return status.Errorf(codes.Internal, "error getting block device for volume %s: %s", volID, err.Error())
		}

		mntFlags := mntVol.GetMountFlags()
		err = mountBlock(sysDevice, targetPath, mntFlags, singleAccessMode(accMode))
		if err != nil {
		...
		err = gofsutil.BindMount(ctx, stagingPath, targetPath, mntFlags...)
		if err != nil {
			return status.Errorf(codes.Internal, "failed to publish NVMe filesystem volume: %s", err.Error())
		}
	}
...
func isAlreadyPublished(ctx context.Context, targetPath string) (bool, error) {
	mounts, err := getPathMounts(ctx, targetPath)
	if err != nil {
